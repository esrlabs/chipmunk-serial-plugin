import {
  ...
  IOptions,
  IIOState,
} from "../controllers/controller.serialport";
...
class ServicePorts {
  ...
  public destroy(): Promise<void> {
    return new Promise((resolve) => {
      ...
      Promise.all(
        Array.from(this._controllers.keys()).map((port: string) => {
          return this._close(port).catch((error) => {
            this._logger.error(
              `Failed to close port "${port}" due to error ${error.message}`
            );
          });
        })
      )
        .catch((error: Error) => {
          this._logger.error(
            `Failed to close all ports due to error ${error.message}`
          );
        })
        .finally(() => {
    ...
    return new Promise((resolve, reject) => {
      if (typeof options !== "object" || options === null) {
        return reject(
          new Error(
            this._logger.error(
              `Fail to get port controller because options isn't ann object`
            )
          )
        );
      }
      if (typeof options.path !== "string" || options.path.trim() === "") {
        return reject(
          new Error(
            this._logger.error(
              `Fail to get port controller because "path" is incorrect: ${options.path}`
            )
          )
        );
      }
      let controller: ControllerSerialPort | undefined = this._controllers.get(
        options.path
      );
      if (controller !== undefined) {
        this._refPortListeners(session, options.path, listeners)
          .then(resolve)
          .catch((error: Error) => {
            return reject(
              new Error(
                this._logger.error(
                  `Fail to get port controller due to error: ${error.message}`
                )
              )
            );
          });
      ...
      controller
        ...
        .then(() => {
          ...
          controller.on(
            ControllerSerialPort.Events.data,
            this._onData.bind(this, options.path)
          );
          controller.on(
            ControllerSerialPort.Events.error,
            this._onError.bind(this, options.path)
          );
          controller.on(
            ControllerSerialPort.Events.disconnect,
            this._onDisconnect.bind(this, options.path)
          );
          this._controllers.set(options.path, controller);
          this._refPortListeners(session, options.path, listeners).then(
            resolve
          );
        })
        .catch(reject);
  ...
  public write(port: string, chunk: Buffer | string): Promise<void> {
    return new Promise((resolve, reject) => {
      let controller: ControllerSerialPort | undefined =
        this._controllers.get(port);
      if (controller === undefined) {
        return reject(
          new Error(
            this._logger.error(
              `Fail to write into port, because controller of port "${port}" isn't created.`
            )
          )
        );
      }
    ...
    return new Promise((resolve, reject) => {
      SerialPort.list()
        ...
        .catch((error: Error | null | undefined) => {
          reject(
            this._logger.error(
              `Fail to get list of ports because of: ${
  ...
  private _close(port: string): Promise<void> {
    return new Promise((resolve, reject) => {
      let controller: ControllerSerialPort | undefined =
        this._controllers.get(port);
      if (controller === undefined) {
        return reject(new Error(`Controller for ${port} not defined`));
      }
      controller
        .destroy()
        .catch(reject)
  ...
  private _refPortListeners(
    session: string,
    port: string,
    listeners: IListeners
  ): Promise<void> {
    return new Promise((resolve) => {
      let stored: Map<string, IListeners> | undefined =
        this._listeners.get(port);
      if (stored === undefined) {
        stored = new Map();
      }
  ...
  private _onData(port: string, chunk: Buffer) {
    const listeners: Map<string, IListeners> | undefined =
      this._listeners.get(port);
  ...
  private _updateControllerSignatureState(session: string) {
    ...
    this._listeners.forEach(
      (listeners: Map<string, IListeners>, port: string) => {
  ...
  private _sendConnectedPortsState() {
    if (typeof this._token !== "string") {
      this._logger.warn("Cannot send port's state due to wrong type of token.");
      return;
    }
